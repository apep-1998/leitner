...
class WordForm(ModelForm):
    ...
    def clean_categorys(self):
        data = self.cleaned_data["categorys"]

        like_words = WordModel.objects.filter(word=self.cleaned_data["word"])
        for word in like_words:
            if word.categorys == data:
                raise ValidationError("this word is exsist")
...
class AddItem2Box(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['box'].queryset = LeitnerBoxModel.objects.filter(user=self.user)
...
class SpellForm(forms.Form):
    ...
    spell = forms.CharField(max_length=20, required=True, widget=forms.TextInput({"placeholder": "spell", "autofocus":''}))
